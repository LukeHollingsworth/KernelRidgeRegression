...
def AllAttributes(trainingData, testData):
    X_training = np.zeros((len(trainingData['CRIM']), 12))
    X_test = np.zeros((len(testData['CRIM']), 12))
...
# Calculate MSE given, optimal parameters, averaged over 20 runs
def MeanSquareErrors():
    ...
    for i in range(20):
        ...
        mseTrain += MSE(X_training, preds, Y_training)

        # Test Data
        alpha = KernelRegressionTrain(X_training, Y_training, minGamma, minSigma)
        preds = KernelRegressionPredict(X_training, X_test, alpha, minSigma)
        mseTest += MSE(X_test, preds, Y_test)

    print(np.sqrt(mseTrain/20))
    print(np.sqrt(mseTest/20))


# Calculate SD, given optimal parameters, averaged over 20 runs
def StandardDeviations():
    sdTrain = sdTest = 0
    mseTrainAverage = 10.5
    mseTestAverage = 12.3
    for i in range(20):
        trainingData, testData = GenerateData()
        minGamma, minSigma, minMSE, meanResults = OptimalParameters(trainingData, testData)
    ...
    print(np.sqrt(sdTrain/20))
    print(np.sqrt(sdTest/20))