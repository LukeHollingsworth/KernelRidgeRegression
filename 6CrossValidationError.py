...
def GenerateData():
    ...
    with open('Question 5/boston_filter.csv', mode='r') as infile:
...
def SelectData(trainingData, testData, feature):
    ...
    # Append X values to training data
...
def OptimalParameters(trainingData, testData):
    ...
    plotError = np.zeros((len(Gamma), len(Sigma)))
    ...
    # Return x, y and z data for plotting
    return Gamma, Sigma, plotError

# Generate heatmap plot from x, y and z data 
trainingData, testData = GenerateData()
Gamma, Sigma, plotError = OptimalParameters(trainingData, testData)
np.savetxt('plotError', plotError)

# State range of x and y axis to be plotted
plt.imshow(np.log(plotError), cmap='hot', extent=[26, 40, 7, 13])
plt.xlabel('Gamma')
plt.ylabel('Sigma')
plt.show()
